...
CryptoolWindow::CryptoolWindow(QWidget *parent)
    : QMainWindow(parent)
{
    ui.setupUi(this);
	...
	connect(ui.pButton, &QPushButton::pressed, this, &CryptoolWindow::selectPlainFile);
	connect(ui.cButton, &QPushButton::pressed, this, &CryptoolWindow::selectCipherFile);
	connect(ui.kButton, &QPushButton::pressed, this, &CryptoolWindow::selectKeyFile);
	connect(ui.encryptBtn, &QPushButton::pressed, this, &CryptoolWindow::start_encrypt);
	connect(ui.decryptBtn, &QPushButton::pressed, this, &CryptoolWindow::start_decrypt);
}

CryptoolWindow::~CryptoolWindow()
{}

void CryptoolWindow::yield(QString text) {
	ui.statusLabel->setText(text);
}

void CryptoolWindow::yield(const char* text) {
	QString txt(text);
	ui.statusLabel->setText(txt);
}

QString CryptoolWindow::selectFile() {
...
void CryptoolWindow::start_decrypt() {
...
bool CryptoolWindow::encrypt(const char* pt_path, const char* ct_path, unsigned char* key) {
	if (!check_key()) {
		return false;
	}
	symmetric_key skey;
...
bool CryptoolWindow::decrypt(const char* ct_path, const char* pt_path, unsigned char* key) {
	if (!check_key()) {
		return false;
	}
	symmetric_key skey;
